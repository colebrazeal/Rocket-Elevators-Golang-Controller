...
func NewColumn(_id int, _amountOfFloors int, _amountOfElevators int, _servedFloors []int, _isBasement bool) *Column {
	return &Column{_id, "active", _amountOfFloors, _amountOfElevators, 0, _servedFloors, _isBasement, *createElevators(_amountOfFloors, _amountOfElevators), *createCallButtons(_amountOfFloors, _isBasement)}
}
...
func (c *Column) findElevator(requestedFloor int, _requestedDirection string) *Elevator {
	...
	if requestedFloor == 1 {
		...
		for _, elevator := range c.elevatorsList {
			...
			if elevator.currentFloor == requestedFloor && elevator.status == "stopped" && _requestedDirection == elevator.direction {
				bestElevatorInformations = *checkIfElevatorIsBetter(1, elevator, bestElevatorInformations, requestedFloor)
